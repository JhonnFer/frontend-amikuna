...
const useEventos = ({ autoCargar = true, onAsistenciaSuccess } = {}) => {
  ...
  const obtenerEventos = useCallback(async () => {
    ...
    try {
      ...
      setEventos(eventosFuturos); // ← ya filtrado desde el inicio
    } catch (err) {
  ...
  const confirmarAsistencia = useCallback(
    async (idEvento) => {
      if (!idEvento) {
        setErrorAsistencia("ID de evento inválido");
        return;
      }
    ...
    [onAsistenciaSuccess],
  ); // ✅ solo onAsistenciaSuccess que sí puede cambiar
